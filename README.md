...
## 🧪 Independent CLI Module Execution Commands

Every module in `src/` can be executed independently from the terminal:

1. **Vehicle Detection**:
   ```bash
   python src/detect.py "videos/uploaded/example.mp4"
   ```
   *Output*: `output/detected/<video>_detected.mp4` & `output/detections/<video>.json`

2. **Accident Detection**:
   ```bash
   python src/accident.py "output/detections/example.json"
   ```
   *Output*: `output/accidents/<video>_accident.json`

3. **Severity Classification**:
   ```bash
   python src/severity.py "output/accidents/example_accident.json"
   ```
   *Output*: `output/severity/<video>_severity.json`

4. **Fire Detection**:
   ```bash
   python src/fire.py "videos/uploaded/example.mp4"
   ```
   *Output*: `output/fire/<video>_fire.json`

5. **Smoke Detection**:
   ```bash
   python src/smoke.py "videos/uploaded/example.mp4"
   ```
   *Output*: `output/smoke/<video>_smoke.json`
...
6. **Emergency Recommendation**:
   ...
   ```
   *Output*: `output/emergency/<video>_emergency.json`
